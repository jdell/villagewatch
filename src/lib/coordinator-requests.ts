 ...
 *
 * This module owns the **applied-for** route to `COORDINATOR`: a resident asks,
 * a platform administrator decides, and the promotion is written here rather
 * than at either call site — the API route and the admin queue's server action.
 * A promotion is the moment somebody gains the ability to read their neighbours'
 * verbatim reports and to decide what the village sees, and it carries
 * obligations that are easy to do twice and easier to forget once.
 *
 * **It is not the only place a role is raised, and saying so was wrong.**
 * `appointCoordinator` in `src/lib/villages.ts` is the other, and it exists for
 * a reason the application flow structurally cannot cover: an application comes
 * *from* a resident of a village, so a village whose first coordinator does not
 * exist yet has nobody who can file one. The two share their rules by convention
 * rather than by code — never demote somebody who has since gained more access,
 * and fill `verifiedAt` only when it is empty — so a change to either is a change
 * to check against the other.
 *
 * A third copy used to sit at the bottom of this file: an `appointCoordinator`
 * and a `removeCoordinator` reachable only from the singular village module,
 * which nothing imported. Both are gone.
 *
