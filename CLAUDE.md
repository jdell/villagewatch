...
src/
  ...
  app/
    ...
    (app)/                    Authenticated shell (sidebar); force-dynamic
      ...
      dashboard/queue/        The review queue on its own screen — pending
                              reports with Publish / Edit / Reject, and what is
                              already published, collapsed. Where a coordinator
                              actually spends their time
      dashboard/settings/     Every village setting in one place, plus the
                              invite and the resident list. The four decisions
                              that used to sit under the queue
      dashboard/actions.ts    Moderate, audited raw-text reveal, the four
      ...
      reports/                Community safety report for police or the council
                              ...
                              Also the weekly summary history, which is the one
  ...
  components/                 Shared UI (logo, app-shell, placeholder, auth forms)
    ...
    dashboard/stat-card.tsx   One figure with its trend against last period —
                              ...
                              cards have no period to trend against
...
docs/                         The documents rendered from disk, not restated
  COORDINATOR_DASHBOARD_REDESIGN.md  Why the coordinator's screens are five
                              tabs and what moved where. Read by people,
                              rendered by nothing — no tracing line
...
between the typecheck and the build. Forty-seven files, 803 tests, covering the
...
directives, what may and may not reach a public social post, and the village
suspension pair — both admin lists, one status in and out of each transition,
and the service read that blocks without claiming a village is suspended.
...
the code. This is the part a later change has to keep true.

`/dashboard` used to be one nine-hundred-line Server Component doing four jobs:
figures, the review queue, five village settings forms and the invite panel. It
is three routes now, and with `/map` and `/reports` the coordinator has five
...
- **`/map` is deliberately not moved under `/dashboard`.** It is the one of the
  five residents see too, and it is the same screen for both — moving it would
  either give residents a coordinator-shaped URL or fork the page in two.
- **Overview writes nothing**, which is a property rather than an accident: it
  is the page a coordinator leaves open, and it is now safe to revalidate under
  them. Everything that writes is on Queue or Village settings.
- **`isActive` takes the longest matching prefix, not every matching one.** The
  four nested tabs live under `/dashboard`, so a plain `startsWith` lights up
  Overview at the same time as Queue, Village settings, Compliance and the
  Guide — five highlighted rows and no way to tell which page you are on. The
  fallback is deliberate too: `/dashboard/audit` has no nav item of its own and
  falls back to Overview, which is where its link is.
- **The Queue item carries a pending badge**, computed in `(app)/layout.tsx`
  alongside `isAdmin` and for the same reason — the shell is a Client Component
  and this is a database read. Gated on the role, because a resident has no
  Queue item to badge and `PENDING_REVIEW` is a status they are not shown
  (domain rule 6). **Absent rather than zero**: a badge reading "0" beside a
  navigation item reads as a thing to go and look at.
- **Two of Overview's four stat cards are deliberately outside the period
  control**, and say so on their own labels. "Waiting for review" is every
  unreviewed report all time — a pending figure that fell to zero because
  somebody selected "Last 7 days" would be the one number in the app that could
  say the work is done when it is not — and "Active residents" is a state rather
  than a rate. That is what `StatCard`'s optional `previous` is for; passing a
  trend for either would be a comparison against a window neither is measured
  over, which is worse than no trend because it looks like one.
- **"Active" means the account is open**, not that anybody used it.
  `User.lastActiveAt` exists and nothing writes it, so a figure built on that
  column would read as engagement and be zero for every village.
- **`revalidatePath` targets moved with the pages**, and this is the half of the
  split easiest to get wrong: a settings action that still revalidated
  `/dashboard` would save correctly and leave the old value on screen. The
  moderation action revalidates Queue *and* Overview (two cards move), the
  auto-approve action revalidates Settings *and* Queue (the notice that takes
  the queue's place), and the channel action revalidates Queue too — the "Open
  WhatsApp" button on a freshly approved report reads that column.
- **`TIME_RANGES` gained `365`.** Comfortably inside `MAX_CUSTOM_RANGE_DAYS`
  (730), so nothing about the custom-range clamp moved.
  `tests/date-range.test.ts` already asserts every value in
  `DASHBOARD_RANGE_VALUES` is a known preset, which is what catches one being
  added to a list and not the other.

### The Queue tab's Edit button

The one authorisation change in the redesign, and worth reading before assuming
it was always there.

`/incidents/[id]/edit` was the **reporter's**: `requireSession()`, then
`reporterId: session.user.id` and the queue statuses in the predicate. A
coordinator could not reach it for somebody else's report, so a queued report
with a landmark in it that identifies a house could only be published as filed
or rejected outright.

It is now **a coordinator of the same village, on a report still in the queue**,
...
- **`rawDescription` is untouched** — the form edits five public columns, and
  ...
  own edit.
- **Written as a dropped clause rather than an `OR`**, so the reporter's own
  path is byte-for-byte what it was, and ownership stays in the `where` rather
  than moving to a prior read.
- **Audited on the existing action.** `incident.edit` already carried
  `actorRole`; the row gains `byCoordinator` because a coordinator editing their
  *own* report satisfies both, and the trail should say which hat was worn.

`/terms` §7 has said "coordinators may edit, reject, resolve or archive any
report in their village" since it was written, so this makes the code match the
notice rather than the other way round.

### The resident list

`listVillageResidents` and `setResidentRole` in `src/lib/villages.ts`,
`ResidentList` on the Settings tab. This is the **third** place in the codebase
that writes `User.role` and the first that is not platform-admin only, which is
why the rules live in the module rather than at the call site.

- **It writes one of two roles and never `COORDINATOR`.**
  `RESIDENT_MANAGED_ROLES` is the pair, and the form posts an *intent* rather
  than a role (domain rule 5) — there is no shape of request that can ask for
  coordinator access. The other two routes to that role are
  `decideCoordinatorRequest` and `appointCoordinator`, both platform-admin only,
  and a coordinator who could mint coordinators would make that review
  decorative.
- **`RESIDENT_MANAGED_ROLES` is written out rather than derived from
  `COORDINATOR_APPLICANT_USER_ROLES`**, which happens to hold the same pair
  today for an unrelated reason — the roles that may still *apply*. Sharing the
  constant would make a change to who can apply a silent change to who can be
  verified.
- **Five refusals**: not a coordinator caller, not yourself, not anybody holding
  coordinator access, not a resident of another village, not a closed account.
  The self case is what stops a coordinator writing `RESIDENT` over their own
  row and demoting themselves out of the page they are standing on; the
  coordinator case is so that two coordinators who fell out cannot remove each
  other. Each is asserted in `tests/resident-role.test.ts`.
- **`verifiedAt`/`verifiedById` follow the role in both directions**, which
  differs from `appointCoordinator` — that fills them only when empty and never
  clears them, because there verification is a side effect of a different
  decision. Here it *is* the decision.
- **Audited as `village.resident_role_changed`**, toned `sensitive`, carrying
  both roles and neither the name nor the address: `entityId` resolves to the
  row, and the trail is readable by every coordinator in the village for as long
  as the village exists.
...
- **`PatternAlert` rows are rendered now, and only read.** The dashboard
  redesign gave them two surfaces: Overview counts the alerts raised in the
  selected period, and `/reports` lists the most recent
  `WEEKLY_SUMMARY_HISTORY_SIZE` of them as the village's weekly summary history.
  Both are reads. **Acknowledge and dismiss still have no UI** — `acknowledgedAt`
  and `dismissedAt` are columns nothing writes, and the RLS UPDATE policy is
  still in place waiting on a screen. Acting on an alert is a decision with a
...
- **Resident verification has a UI now**, on `/dashboard/settings` — the
  ...
  "The resident list" below for the five refusals and why each one is there.
  **Nobody has ever been verified through it**, because no village has ever been
  activated. What is still missing is the other half this entry used to ask for:
  there is no *join request* to approve — a resident with the join code is in the
  village the moment they register, and verification is a note made afterwards
