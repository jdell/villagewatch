...
**Last updated:** 13 August 2026 (T1 and T2 closed; L6 closed — all ten
migrations are applied; L3's join code is now actually enforced, which it was
not despite this file saying so since 27 July. Earlier: B1, B2, B4, B5, I1, I2,
I3, N1, N9, N13, N14, T3, T4, T7, T8 closed; DPIA drafted; compliance gate and
the per-village face redaction level added; Article 28(3) processing agreement
drafted and added to the gate as L8)
...
| T10 | Original wording survives archiving | Medium | `/privacy` said the reporter's verbatim words were deleted when a report was archived at 12 months. They are not — the retention job's archive step is `status: "ARCHIVED"` and touches no other column. Fixed on 13 Aug by correcting the notice, which is the honest half; the *other* half is a product decision nobody has made. Clearing `rawDescription` on archive would be a one-line change to `archiveExpiredIncidents` and would match what the notice used to promise. Decide which, and if it is deletion, note that the tombstone shape already exists in `src/lib/erasure.ts`. |
