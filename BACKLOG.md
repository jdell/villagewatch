...
| N15 | Report footers name the deployment constant in a community village | Not started | `reportController` falls back to `DATA_CONTROLLER` — "[Parish Council name]" — and `Village.parishCouncil` is labelled "parish council" on the dashboard. In a community village the controller is the coordinator, so the field should be labelled for both cases and the `/reports` warning should say so. The Coordinator Guide already tells a community coordinator to put the group's name in it; the screen does not. |
