...
| `feat/observability` | **In review.** The three v1.0.0 observability gaps, over the Slack webhook that already existed rather than a new processor. All four crons report their outcome — **success as well as failure**, because nothing here can detect a job that never fired and a line on every run is what makes a missing one mean something. `/api/health` is committed and is a **readiness** probe: it runs `SELECT 1` and answers **503** when Postgres does not, since a 200 carrying `"degraded"` in the body is what keeps a dashboard green through an outage. `src/instrumentation.ts` reports server errors through `onRequestError`, skipping redirect and not-found. The three `error.tsx` comments were corrected: they are Client Components and log to the resident's own browser, which the comments denied. No migration, no schema change, **no new environment variable** — `SLACK_WEBHOOK_URL` was already in `.env.example`. Build, typecheck, lint and 840 tests pass. Verified against a dev server: the endpoint answers, a thrown route alerts once, and the duplicate is suppressed. | Review, merge. Then point a monitor at `/api/health` and watch the first retention run report itself |
