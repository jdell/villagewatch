...
| `feat/sentry` | **In review.** Sentry (`@sentry/nextjs` v10) on the **EU** instance, alongside the Slack alerting rather than replacing it — Sentry gets the error, Slack gets the line somebody watches. Three runtime configs through one shared `beforeSend` (`src/lib/sentry-scrub.ts`), `withSentryConfig` for source maps, and `tunnelRoute: /api/monitoring`, which also sidesteps the enforced CSP. **Four documents changed with it** — `/privacy` §6, both processing agreements' sub-processor lists and `docs/DPIA.md` §5 (plus a new §5.4a assessing the one residual risk: an exception message can quote what the code was handling). **Nothing has ever reached Sentry** — no project exists and every variable is empty, which leaves the SDK inert. Build, typecheck, lint and 851 tests pass, including a build with the whole Sentry block unset, the way CI runs it. | Joel: create the project, then set the six variables in Vercel **and redeploy** — `NEXT_PUBLIC_` is inlined at build time. Then throw something and check both a readable stack trace and the Slack line |
