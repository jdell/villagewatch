...
| `fix/anonymous-by-default` | **In review.** Also carries the charts on `/reports` (inside the print region, `printable`), the resident summary on `/incidents`, and **the same three charts in the downloaded PDF** — drawn as flexbox `View`s, because `@react-pdf/renderer` has no DOM and Recharts cannot run in it, and fed the same `getIncidentTrend` over the same range as the page so the file and the screen cannot disagree.  The wizard's "file anonymously" box starts ticked, in the form and in `incidentReportSchema`. Investigating it turned up that the flag hides the reporter from the **coordinator's queue** and from nobody else — no resident-facing surface has ever rendered a reporter's name — so the old label was wrong in both halves and was rewritten with the default. **`/privacy` §6 is wrong about this in two places and was before this change**; it needs a decision rather than a patch, and the PR says which two. | Review, merge. Then decide the `/privacy` question |
