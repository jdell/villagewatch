...
| `feat/pdf-charts` | **In review.** The three summary charts in the downloaded PDF, drawn as flexbox `View`s — `@react-pdf/renderer` has no DOM, so Recharts cannot run in it, and `Svg` would be a second layout engine to draw rectangles. Fed the same `getIncidentTrend` over the same range as the on-screen chart, so the file and the page cannot disagree. They **replace** the summary's two `CountTable`s and print every figure those did; `CountTable` stays for the police panel, which is deliberately two counts and never one chart. No migration, no schema change, no new environment variable. Build, typecheck, lint and 825 tests pass. **The charts have only ever drawn fixtures** — there is still no `ACTIVE` village. | Review, merge |
