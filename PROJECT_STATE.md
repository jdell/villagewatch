...
> **Parts of this file were stale when the social digest landed, and are flagged
> rather than guessed at.** `package.json` on `main` reads `0.1.58` while the
> Current status table below describes `v0.1.51` and names the releases up to it,
> and `feat/ecops-rss-integration` is listed as "in review" though it merged as
> PR #28. Reconciling the seven releases in between wants whoever made them —
> inventing the list would put fabricated facts in the one file that is believed
> because it is a status file. The rows this change actually touches are current.
...
| `feat/social-digest` | **In review.** The weekly social digest: `src/lib/digest/format-social-post.ts`, `GET /api/digest/social`, a "Copy weekly post" button on Overview, 24 tests, and `docs/FACEBOOK_LAUNCH_GUIDE.md`. No migration and no schema change — it reads three columns of already-published reports. `/privacy` §6 gained an entry and `LEGAL_LAST_UPDATED` moved to 7 September with it. Build, typecheck, lint and 762 tests pass. **Nothing has ever been posted to Facebook from it**, and the button has never been pressed against a village with a week of reports in it — there is still no `ACTIVE` village. | Review, merge. No migration to apply |
