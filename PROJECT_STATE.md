...
| `feat/dashboard-charts` | **In review.** Charts on the Overview tab — a trend over the selected period, the category breakdown and the severity split — plus a daily strip above the activity feed and a ranked village comparison on `/admin/villages`. Adds **Recharts**, the first charting dependency in the repo, deferred behind one `next/dynamic` boundary so it stays off the initial payload. **No migration and no new query shape a coordinator could not already run**: everything is counted from `incidents` and `users` over predicates the dashboard already used. Build, typecheck, lint and 803 tests pass. **No chart has ever drawn a real village's reports** — there is still no `ACTIVE` village — so what has been verified is the rendering against fixtures, at desktop and at iPhone-portrait width. | Review, merge. Nothing to apply |
