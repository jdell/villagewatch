...
**Last updated:** 8 September 2026 · **Repo version:** `v0.1.59` · **Branch:**
`main` · **Domain:** https://villagewatch.app

> **Parts of this file are stale, and are flagged rather than guessed at.**
> `package.json` on `main` reads `0.1.59` while the Current status table below
> describes `v0.1.51` and names the releases up to it, and
> `feat/ecops-rss-integration` is listed as "in review" though it merged as
> PR #28. Reconciling the releases in between wants whoever made them —
> inventing the list would put fabricated facts in the one file that is believed
> because it is a status file. The rows the social digest and village suspension
> touched are current; both branches wrote a version of this note, and this is
...
| Test suite | Vitest, **53 files, 900 tests**, all passing (~3.6s; the pacer test spends 3s of that genuinely measuring the wait) — runs with no `.env.local` and no database. Unit only bar two component tests, both rendered to a string with no DOM: `period-control.test.tsx` and `legal-placeholders.test.tsx`. Five route handlers are now covered — retention, the vote, **`POST /api/incidents`**, which closed the gap this file and two others named for a month, `/api/health`, and the interest archive route, which is there for its gate |
...
| Landing exactly `v1.0.0` | `version.yml` owns the version: it runs on any push to `main` with a releasable commit and bumps *from* whatever `package.json` says. A hand-set `1.0.0` merged with an ordinary merge commit therefore releases **1.1.0**, not 1.0.0. The workflow skips when the head commit message starts with `chore(release):` or contains `[skip ci]` — so a **rebase merge** (whose head commit is then `chore(release): 1.0.0`) lands exactly 1.0.0, and so does a merge commit carrying `[skip ci]`, which also skips the Vercel production deploy and wants one triggering by hand afterwards. Tagging `v1.0.0` before the merge works too: the job finds no releasable commits since the tag and does nothing |
...
| `feat/interest-archive` | **In review.** Archiving an interest registration with a reason, a Pending/Archived filter on `/admin/villages`, and **no hard delete** — migration 18 adds `status`, `archived_at` and `archived_reason`. The status, date and reason are one statement, and the pending guard is in the `where`, so a second press cannot re-stamp a row. **Restore was not in the brief and is included**: an archive with no way back is the gap `suspendVillage` exists to close. RLS needed no change and `rls_policies.sql` records why. Verified against a throwaway PostGIS — 18 migrations apply, no drift, archive/restore/second-press exercised on real rows — and the panel driven in a browser, where an unauthenticated submit was refused by the route's own gate. Build, typecheck, lint and 900 tests pass. | Review, merge. Then **apply migration 18 and re-run `rls_policies.sql`** |
| `feat/interest-registration` | Merged as PR #39, 13 September. Released as **`v1.5.0`**. "My village isn't listed" on the sign-up form, and the expansion pipeline on `/admin/villages`. | Delete |
| `feat/sentry` | Merged as PR #38, 12 September. Released as **`v1.4.0`**. Sentry on the EU instance, alongside the Slack alerting. **No project exists yet and every variable is empty**, so the SDK is inert until a DSN is pasted in. | Delete |
| `feat/observability` | Merged as PR #37, 12 September. Released as **`v1.3.0`**. Cron outcome alerts, `/api/health` as a readiness probe, and `src/instrumentation.ts`. **`SLACK_WEBHOOK_URL` is not set in Vercel**, so all of it logs instead of posting in production — see the open item below. | Delete |
| `feat/pdf-charts` | Merged as PR #36, 11 September. Released as **`v1.2.0`**. The three summary charts in the downloaded PDF, drawn as flexbox `View`s — `@react-pdf/renderer` has no DOM, so Recharts cannot run in it. **The charts have only ever drawn fixtures**; there is still no `ACTIVE` village. | Delete |
| `fix/anonymous-by-default` | Merged as PR #35, 11 September, by rebase. Released as **`v1.1.0`** — the merge carried releasable commits and `version.yml` bumps *from* `package.json`, which read 1.0.0, so this is the minor bump that row predicted rather than a mistake. The wizard's "file anonymously" box starts ticked; the flag hides the reporter from the **coordinator's queue** and from nobody else, and Joel settled that question the other way — coordinators always see who filed, `/privacy` says so, and `isAnonymous` is now inert on every screen. Also carried the charts on `/reports` and the resident summary on `/incidents`. | Delete. **Open question left behind:** `isAnonymous` is recorded and rendered nowhere — decide whether it stays a preference, gains a resident-facing byline, or comes off the form |
| `feat/v1-hardening` | **In review.** The three things blocking 1.0.0, as three commits: rate limits on `/api/auth/login` and `/api/auth/register` (address-keyed, through the Postgres limiter rather than a Map — see the commit), `global-error.tsx` and `(app)/error.tsx`, and `npm run db:setup` for the migrations/PostGIS/RLS ordering. Bumps to **1.0.0**. No migration and no schema change. Build, typecheck, lint and 820 tests pass. **`version.yml` will bump past 1.0.0 on a merge commit** — see the note under Current status. | Review, merge by rebase or with `[skip ci]`, or accept 1.0.1 |
| `fix/auth-verification-messages` | **In review.** The two places the app said the wrong thing to somebody who had registered and not clicked their confirmation link: the sign-in form told them their password was wrong, and the register form's "check your email" was a toast fired immediately before a redirect. Both are panels now, and `isEmailNotConfirmedError` is the narrow predicate behind the first — **read its header before touching it**, because the sentence is only not an enumeration oracle by virtue of GoTrue checking the password before the confirmation state, which nothing in this repository can test. No migration, no schema change, no new environment variable. 47 files, 809 tests with `main` merged in. **Untested against a real Supabase** — there is no `ACTIVE` village and nobody has ever registered, so the unconfirmed branch has never been reached by a real sign-in. | Review, merge. Worth confirming against a real unconfirmed account on the preview |
| `feat/dashboard-charts` | Merged as PR #32, 11 September. Released as `v0.1.62`. Charts on the Overview tab and a ranked village comparison on `/admin/villages`, over **Recharts** — the first charting dependency in the repo, deferred behind one `next/dynamic` boundary. **No chart has ever drawn a real village's reports** — there is still no `ACTIVE` village — so what was verified is the rendering against fixtures, at desktop and at iPhone-portrait width. | Delete |
| `feat/admin-village-disable` | **In review.** Suspend and reactivate a village from `/admin/villages`, super-administrator only: `suspendVillage` / `reactivateVillage` / `getVillageServiceState` in `src/lib/villages.ts`, a Suspended tab, a resident banner above every authenticated page, and gates on both report routes and the wizard host. **No migration** — `VillageStatus.SUSPENDED` has existed since the first one; what was missing was anything in the application that could write it. Build, typecheck, lint and 789 tests pass on `main` merged in. **Nothing has ever been suspended** — there is still no `ACTIVE` village to suspend. | Review, merge. No migration to apply |
| `feat/social-digest` | Merged as PR #30, 8 September. Released as `v0.1.59`. **Nothing has ever been posted to Facebook from it** — see the open item below. | Delete |
...
### Four server-only variables are missing from Vercel — 12 September 2026

Found by reading `vercel env ls production` while wiring Sentry, not by anything
failing. **Every one of these fails closed and silently**, which is by design in
each module and means production has been quietly refusing things nobody has
looked at.

| Variable | What is unset in production means |
|---|---|
| `SLACK_WEBHOOK_URL` | Every alert PR #37 added **logs instead of posting**. The cron reports, the server-error line, all of it — written to a Vercel function log and read by nobody, which is the exact state that work existed to end |
| `CRON_SECRET` | `isCronAuthorised` fails closed with no secret set, so **all four scheduled routes refuse every request**. Nothing has been archived, no digest written, no police month fetched |
| `ADMIN_EMAILS` | `isPlatformAdmin` fails closed, so `/admin` **refuses everybody** while coordinator applications keep arriving. `submitCoordinatorRequest` logs a warning each time it finds nobody to tell |
| `SUPER_ADMIN_EMAILS` | The merge screen and the suspend buttons are unreachable. This one is arguably correct as-is — there is no default on purpose — but it should be a decision rather than an oversight |

None of this is a code change and none of it is in this repository: it is four
entries in the Vercel dashboard followed by a redeploy. The first two are what
make the observability work real rather than theoretical, and the third is the
one a resident is currently waiting on.

There is also a `NEXT_PUBLIC_URL` in Vercel that appears in no file in this
repository — `NEXT_PUBLIC_APP_URL` is the one the code reads. Worth deleting
once somebody has confirmed nothing external depends on it.
...
**One interaction the two September branches create between them**, found while
merging `main` into this one and deliberately not fixed in that merge commit:
`GET /api/digest/social` does not read `Village.status`, so a coordinator of a
*suspended* village can still copy a weekly post whose "Join your village" link
points at a village `checkVillageJoin` will refuse on status. Gating the route
would be the wrong fix — the post is a read of already-published data, and
suspension preserves read access on purpose — so the answer is either to drop
the join line from the post while a village is out of service, or to leave it
and accept that a suspension is temporary and the coordinator has a banner
telling them so. It is a dead link on a public feed either way, which is why it
is written down rather than left to be discovered.

### The weekly social digest is built and has never been posted — 7 September 2026

`GET /api/digest/social` formats one village's week and
`CopyWeeklyPostButton` on Overview puts it on a coordinator's clipboard.
`docs/FACEBOOK_LAUNCH_GUIDE.md` is the operational half — the Page, the groups,
the intro post and what to track.

**Three things are untested by anything but the unit suite**, and the first is
the one that would embarrass a launch:

1. **There is still no `ACTIVE` village**, so the button has never been pressed
   against a week of real reports. Against an empty village it produces the
   "nothing reported this week" post, which is correct and is not the case worth
   checking. Activate a village first — `npm run db:activate-village` — and take
   the coordinator through `/dashboard/compliance`, or every join link in the
   post lands on a page telling residents to contact their coordinator.
2. **No post has ever been pasted into Facebook.** Read what lands: Facebook
   reflows line breaks in a way nothing in this repository can predict, and the
   emoji in the digest are doing structural work rather than decoration.
3. **The join link carries the village's join code.** That is by design — see
   The village invite — but a post on a public feed is the widest place that code
   has ever been, and `regenerateJoinCode()` is the answer if it needs rotating.

No migration and no schema change: it reads `type`, `severity` and
`locationText` off already-published reports.
