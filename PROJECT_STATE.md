...
| Test suite | Vitest, **52 files, 867 tests**, all passing (~3.6s; the pacer test spends 3s of that genuinely measuring the wait) — runs with no `.env.local` and no database. Unit only bar two component tests, both rendered to a string with no DOM: `period-control.test.tsx` and `legal-placeholders.test.tsx`. Four route handlers are now covered — retention, the vote, **`POST /api/incidents`**, which closed the gap this file and two others named for a month, and `/api/health` |
...
| `feat/interest-registration` | **In review.** "My village isn't listed" on the sign-up form: a conditional panel, a two-path fork (resident or coordinator candidate), a confirmation screen, and one `village_interest` row — **no account is created**. Migration 17 plus RLS, a confirmation email per path, a Slack alert for candidates only, and an expansion-pipeline section on `/admin/villages` grouped by village with candidates first. `/privacy` §2, §4 and §7 changed and `LEGAL_LAST_UPDATED` moved — it is a new category of personal data about people with **no account**, who therefore have no screen to delete themselves from. **The RLS departs from the brief: `anon` gets no INSERT** — see the note in `rls_policies.sql`. Verified against a throwaway PostGIS: all 17 migrations apply, no drift, `anon` refused both INSERT and SELECT, owner not; and end to end on a dev server — row written, both emails queued, Slack fired for the candidate only. | Review, merge. Then **apply migration 17 and re-run `rls_policies.sql`** |
